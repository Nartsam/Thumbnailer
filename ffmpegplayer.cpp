...
void FFmpegPlayer::init(){
    ...
    mPlayState=PlayerState::StopState; //初播放状态
...
FFmpegPlayer::FFmpegPlayer(QObject *parent):VideoPlayer(parent){
    ffmpeg_output_widget=nullptr; video_label=nullptr;
    init();
}
...
void FFmpegPlayer::play(){
    ...
    mPlayState=PlayerState::PlayState;
    if(lastState!=mPlayState) emit state_changed(mPlayState);
    if(lastState==PlayerState::StopState) start_decoding();
}
void FFmpegPlayer::pause(){
    int lastState=mPlayState;
    mPlayState=PlayerState::PauseState;
    if(lastState!=mPlayState) emit state_changed(mPlayState);
}
void FFmpegPlayer::stop(){
    if(mPlayState==PlayerState::StopState) return;
    //clear
    int lastState=mPlayState;
    mPlayState=PlayerState::StopState;
    if(lastState!=mPlayState) emit state_changed(mPlayState);
    if(this->video_label!=nullptr){
        this->video_label->resize(this->ffmpeg_output_widget->size());
    }
}
...
void FFmpegPlayer::next_frame(){
    qWarning()<<"incomplete function:"<<__func__;
}
void FFmpegPlayer::prev_frame(){
    qWarning()<<"incomplete function:"<<__func__;
}
VideoPlayer::PlayerState FFmpegPlayer::state(){
    return mPlayState;
}
...
int FFmpegPlayer::position_changed_threshold(){
    return UpdateMSec;
}
void FFmpegPlayer::set_output_widget(QWidget *output_widget){
    this->ffmpeg_output_widget=output_widget;
    if(video_label!=nullptr) delete video_label;
    video_label=new QLabel(this->ffmpeg_output_widget);
    video_label->resize(this->ffmpeg_output_widget->size());
    // video_label->setStyleSheet("");// ");
    // video_label->setAlignment(Qt::AlignCenter);
    // video_label->setText("Drag Video Here");
}
const QWidget *FFmpegPlayer::output_widget(){
    return ffmpeg_output_widget;
}
...
QImage FFmpegPlayer::current_image(){
    return currentImage.copy(); //需要返回深拷贝的版本
}
void FFmpegPlayer::start_decoding(){
    ...
    //循环读取视频数据
    while(true){
        if(mPlayState==PlayerState::StopState) break; //播放结束
        if(mNeedSeek){
        ...
        if(mPlayState==PlayerState::PauseState){ //暂停
            delay(300);
        }
        else{ //正在播放
